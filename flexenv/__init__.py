...
def develop(args, env):
    ...
    for x in develop_env:
        for dist in develop_env[x]:
            dists.extend(_resolve_requires(dist, env, packages_dir))
...
def _resolve_requires(dist, env, packagesdir):
    results = []
    for req in dist.requires():
        ...
        for req_dist in req_dists:
            results.extend(_resolve_requires(req_dist, env, packagesdir))
